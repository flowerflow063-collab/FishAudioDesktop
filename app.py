...
class FishAPI:
    ...
    def _error(self, response, action):
        ...
        if isinstance(data, dict):
            ...
            detail = response.text.strip() or "Sin detalles proporcionados por el servidor."
        if status == 401:
    ...
    def test_key(self):
        try:
            ...
            raise RuntimeError(f"No se pudo conectar con Fish Audio para comprobar la API key: {e}")
        if not r.ok:
    ...
    def asr(self, audio_path, language="", timestamps=False):
        ...
        for attempt in range(3):
            ...
            if not isinstance(result, dict) or "text" not in result:
                raise RuntimeError(f"Respuesta inesperada de Fish Audio: {result!r}")
            return result
        if last_error is not None:
            self._error(last_error, "transcripción")
    ...
    def create_model_multipart(self, title, visibility, description, tags, files, texts):
        ...
        try:
            ...
            r = requests.post(API_BASE + "/model", headers=self._headers(), data=data, files=multipart, timeout=(15, 300))
        except requests.Timeout:
...
class App(tk.Tk):
    ...
    def run_bg(self, fn, ok=None, label=None):
        ...
        def worker():
            try:
                result = fn()
                if ok: self.after(0, lambda: ok(result))
    ...
    def _tts(self):
        ...
        def add(label, key, default):
            ...
            ttk.Entry(right, textvariable=v, width=25).pack(anchor="w")
        add("Modelo", "model", "s2-pro"); add("Voice / reference_id", "voice", ""); add("Formato", "format", "mp3")
    ...
    def generate(self):
        ...
        if not text: messagebox.showwarning("Texto vacío", "Escribe el texto que quieres convertir en audio."); return
        ...
        try:
            ...
            messagebox.showerror("Parámetros inválidos", "Los parámetros numéricos deben ser válidos."); return
        latency = self.fields["latency"].get().strip()
        if latency: p["latency"] = latency
        out = self.out.get().strip()
        if not out: messagebox.showwarning("Salida", "Selecciona dónde guardar el audio."); return
        self._pending_out = out
        self.run_bg(lambda: self.api.tts(p, self.fields["model"].get().strip() or "s2-pro"), self._tts_done, "Generando audio…")

    def _tts_done(self, data):
        out = self._pending_out; Path(out).parent.mkdir(parents=True, exist_ok=True); Path(out).write_bytes(data)
        self.status.set("Audio generado")
        if messagebox.askyesno("Listo", "Audio guardado. ¿Abrirlo ahora?"): os.startfile(out)

    def _asr(self):
        f = self.asr_tab; ttk.Label(f, text="Transcripción de audio", font=("Segoe UI", 14, "bold")).pack(anchor="w")
        row = ttk.Frame(f); row.pack(fill="x", pady=15); self.asr_file = tk.StringVar()
        ttk.Entry(row, textvariable=self.asr_file).pack(side="left", fill="x", expand=True); ttk.Button(row, text="Seleccionar audio", command=self.select_audio).pack(side="left", padx=8)
        opts = ttk.Frame(f); opts.pack(fill="x"); self.asr_lang = tk.StringVar(value="es")
        ttk.Label(opts, text="Idioma (código: es, en, zh…)").pack(side="left"); ttk.Entry(opts, textvariable=self.asr_lang, width=15).pack(side="left", padx=8)
        ...
        self.asr_text = tk.Text(f, height=22, bg="#181d27", fg="#e8ecf1", relief="flat"); self.asr_text.pack(fill="both", expand=True)
    ...
    def transcribe(self):
        ...
        if not p: messagebox.showwarning("Audio", "Selecciona un archivo de audio."); return
        self.run_bg(lambda: self.api.asr(p, self.asr_lang.get(), self.timestamps.get()), self.show_asr, "Transcribiendo… (máx. 120 s)")

    def show_asr(self, r):
        self.asr_text.delete("1.0", "end"); self.asr_text.insert("1.0", r.get("text", "")); self.status.set("Transcripción lista")
    ...
    def _voices(self):
        ...
        self.models_list = tk.Listbox(self.voices_tab, bg="#181d27", fg="#e8ecf1", selectbackground="#6d5dfc", height=16); self.models_list.pack(fill="both", expand=True, pady=12)
        self.model_info = tk.Text(self.voices_tab, height=10, bg="#181d27", fg="#e8ecf1", relief="flat"); self.model_info.pack(fill="x"); self.model_data = []

    def load_models(self):
        if self.need_api(): self.run_bg(self.api.models, self.show_models, "Cargando modelos…")

    def show_models(self, r):
        self.model_data = r if isinstance(r, list) else r.get("items", r.get("data", [])); self.models_list.delete(0, "end")
    ...
    def delete_model(self):
        ...
        if messagebox.askyesno("Confirmar", f"Eliminar {mid}?"): self.run_bg(lambda: self.api.delete_model(mid), lambda _: self.load_models(), "Eliminando…")

    def create_clone(self):
        if not self.need_api(): return
        win = tk.Toplevel(self); win.title("Crear clon de voz"); win.geometry("620x520"); vars_ = {}
        def field(lbl, key):
            ttk.Label(win, text=lbl).pack(anchor="w", padx=15, pady=(10, 2)); v = tk.StringVar(); vars_[key] = v; ttk.Entry(win, textvariable=v, width=70).pack(padx=15, fill="x")
        field("Título", "title"); field("Descripción", "description"); field("Visibilidad (public/unlist/private)", "visibility"); vars_["visibility"].set("private"); field("Tags separados por coma", "tags")
        ttk.Label(win, text="Audios de referencia").pack(anchor="w", padx=15, pady=(10, 2)); files = tk.StringVar(); ttk.Entry(win, textvariable=files).pack(padx=15, fill="x")
        def pick(): files.set("|".join(filedialog.askopenfilenames(filetypes=[("Audio", "*.wav *.mp3 *.m4a *.flac *.ogg"), ("Todos", "*.*")])) )
        ttk.Button(win, text="Seleccionar audios", command=pick).pack(padx=15, pady=8, anchor="w")
        ttk.Label(win, text="Textos correspondientes, uno por línea (opcional)").pack(anchor="w", padx=15, pady=(8, 2)); texts = tk.Text(win, height=7, bg="#181d27", fg="#e8ecf1"); texts.pack(padx=15, fill="both", expand=True)
        def go():
            ps = [p for p in files.get().split("|") if p]
            if not vars_["title"].get() or not ps: messagebox.showwarning("Faltan datos", "Título y al menos un audio."); return
            win.destroy(); self.run_bg(lambda: self.api.create_model_multipart(vars_["title"].get(), vars_["visibility"].get() or "private", vars_["description"].get(), vars_["tags"].get(), ps, [x for x in texts.get("1.0", "end").splitlines() if x.strip()]), lambda _: self.load_models(), "Creando clon…")
        ttk.Button(win, text="CREAR MODELO", style="Accent.TButton", command=go).pack(pady=12)

    def _settings(self):
        f = self.settings_tab; ttk.Label(f, text="Configuración", font=("Segoe UI", 14, "bold")).pack(anchor="w")
        ttk.Label(f, text="API key de Fish Audio").pack(anchor="w", pady=(18, 4)); self.key = tk.StringVar(); ttk.Entry(f, textvariable=self.key, show="•", width=70).pack(anchor="w")
    ...
    def save_key(self):
        ...
        if not k: messagebox.showwarning("API key", "Pega tu API key de Fish Audio."); return
        api = FishAPI(k); self.status.set("Comprobando API key…")
        def done(_):
            try:
